...
function MotivationalButton({ handleMotivationalQuotes, isLoading }) {
    return (
      <div>
        <button
          onClick={handleMotivationalQuotes}
          ...
          <FontAwesomeIcon icon={faSpinner} className={`w-8 h-8 ${isLoading ? 'animate-spin text-red-400' : ''}`} />
        </button>
      </div>
    );
}
